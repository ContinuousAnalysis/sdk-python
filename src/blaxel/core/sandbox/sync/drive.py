...
class SyncSandboxDrive(SyncSandboxAction):
    ...
    def mount(
        ...
        with client:
            response = post_drives_mount(client=client, body=body)
            if response is None:
    ...
    def unmount(self, mount_path: str) -> DriveUnmountResponse:
        """
        ...
        with client:
            response = delete_drives_mount(param_path, client=client)
            if response is None:
    ...
    def list(self) -> List[DriveMountInfo]:
        """
